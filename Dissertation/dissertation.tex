...
+ look-at camera!
\subsection{From views to 3D points}
This section introduces the process of determining the location in 3-space of points on the surface of the object based solely on the information gathered from the two views.\\
\linebreak
...
0 & 0 & 1 & 0\end{array} \right)\ \begin{bmatrix}
        ...
        \end{bmatrix}$\\
     \linebreak
Assume $f = 1 => \mathbf{C_{0}} = \left(\mathbf{I}|\mathbf{0} \right )$ \\ 
\linebreak

$C_{0}$ is the camera matrix for a basic pinhole model. In the general case, however,  the camera is not located in the origin of the system. \\
An easy way to map from a coordinate system to another is to perform a rotation and a translation. For a 3x3 rotation matrix \textbf{R} and a translation 3x1 matrix \textbf{t}:
